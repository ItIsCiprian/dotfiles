...
const { install_agent_extension_if_not_installed } = require("./install_agent_extension_if_not_installed")
...
function activate(_) {	
			...
			const decorationOptions = {
				...
				renderOptions: {
					after: {
						...
						backgroundColor: new vscode.ThemeColor('editor.background'), // Background color
						borderColor: new vscode.ThemeColor('editor.foreground'), // Lighter gray border color
						border: '0.1px solid', // Border style
						textDecoration: 'none',
						cursor: 'pointer',
			...
			try {
				...
				thisWebview.webview.onDidReceiveMessage(async (data) => {
					...
					if (data.command) {
						...
						} else if (data.command === "showMultiFileEditorChat") {
							vscode.commands.executeCommand(
								"blackbox.openChat"
							)
							telemetry(eventTypes.other, '', {
								tag: 'clicked-multi-file-button'
							})
  ...
  <body style="margin:0px;padding:0px;overflow:hidden">
	...
	statusBarLeftChatOpen.command = 'blackbox.showChat';
	...
	// cyberCoderStatusBar.command = 'blackbox.openChat';
