...
function activate(_) {	
  ...
  function formatTerminalContent(content) {
	...
	let robocoderPanel = undefined
	vscode.commands.registerCommand("blackbox.openRobocoders", () => {
		const robocoderURL = 'https://robocoders.ai/?vscode=true'
		if (robocoderPanel) { 
			robocoderPanel.reveal(vscode.ViewColumn.Two)
		}else{
			robocoderPanel = vscode.window.createWebviewPanel(
				"robocoders-ai",
				"Robocders AI",
				vscode.ViewColumn.Two,
				{
  ...
  <body style="margin:0px;padding:0px;overflow:hidden">
    <iframe id="myIframe" src="${robocoderURL}" frameborder="0" style="overflow:hidden;height:100vh;width:100%" height="100%" width="100%"></iframe>
  </body>
  </html>`

		robocoderPanel.onDidDispose(() => {
			robocoderPanel = undefined
		})
	})

	const openRobocoder = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Right, 100);
	openRobocoder.text = 'Robocoder';
	openRobocoder.tooltip = 'Open RobocodersAI';
	openRobocoder.command = 'blackbox.openRobocoders';
	openRobocoder.show();
