...
const { analyzeLastLine } = require("../requireMultiline")
	...
	const promise = new Promise(async (resolve, reject) => {
		if (text.length) {
			...
			if (text.length>10){
				...
				let api_url = 'https://www.useblackbox.io'
				
				if (editorContext) {
					if (editorContext.globalState.get("installedDate")){
						if (editorContext.globalState.get("installedDate") > date_jan_1){
							endpoint = newEndpoint
						}
						if (editorContext.globalState.get("installedDate") > updateDateMarch){
							endpoint = latestEndpoint
						}
						if (editorContext.globalState.get("installedDate") > latestUpdate){
							endpoint = newEndpoint
						}
						if (editorContext.globalState.get("installedDate") > dateToMilliseconds("2024-08-31")){
							api_url = 'https://www.codegeneration.ai'
							endpoint = 'autocomplete'
						}
					}
				}
				let lastLineType = analyzeLastLine(text)
				
				let requiresMultiline = false
				if ( lastLineType === 'function' || lastLineType === 'class' || lastLineType === 'endpoint' ){
					requiresMultiline = true
				}
