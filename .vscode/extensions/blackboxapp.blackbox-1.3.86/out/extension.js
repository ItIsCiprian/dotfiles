...
const API_BASE_URL = "https://www.codegeneration.ai";
			...
			messages: [
				{
					role: "system",
					content: `
					You are an assitant that can help with code completion. Never answer in markdown format or use natural language in your answers or introductions. Always answer in code directly.

					Generate code in place for user's request. 
					- The code generation shouldn't contain anything other than text and should replace the part denoted by selection.
					- Only generate the part that is supposed to replace the selection
					- Do not include markdown, just code without any markdown or html etc.
					- Do not generate any natural language, generate code only.
					- Directly start with the code and do not generate the triple backticks and coding language used for markdown.
					- You should strictly answer the user with code`,
				},
				{
					role: "user",
					content: `
...
function activate(_) {	
	...
	if (
		installed_date > dateToMilliseconds("2024-07-01")
	) {
		...
		function updateDecoration(editor) {
			...
			let decorationText = `${shortcutKey} (Ask AI)`
			if (
				installed_date
				&& installed_date > dateToMilliseconds(date_in_editor_chat)
			){
				decorationText = `Chat (${shortcutKey}) / Inline (${shorcutForInlineChat})`
			}
  ...
  <body style="margin:0px;padding:0px;overflow:hidden">
	...
	<script>
        ...
        window.addEventListener('message', event => {
            ...
            }else if (message.type === 'closeWebview' || 'newFile' || 'openTerminal' || 'openCommandPalette' || 'openSideBar'){
